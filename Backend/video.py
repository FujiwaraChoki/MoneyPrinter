...
def save_video(video_url: str, directory: str = "../temp") -> str:
    ...
    try:
        ...
        raise VideoProcessingError(f"Error saving video: {str(e)}")
...
def __generate_subtitles_assemblyai(audio_path: str) -> str:
    ...
    try:
        ...
        raise VideoProcessingError(f"Error generating subtitles using AssemblyAI: {str(e)}")
...
def __generate_subtitles_locally(sentences: List[str], audio_clips: List[AudioFileClip]) -> str:
    ...
    try:
        ...
        raise VideoProcessingError(f"Error generating subtitles locally: {str(e)}")
...
def generate_subtitles(audio_path: str, sentences: List[str], audio_clips: List[AudioFileClip]) -> str:
    ...
    try:
        ...
        raise VideoProcessingError(f"Error generating subtitles: {str(e)}")
...
def combine_videos(video_paths: List[str], max_duration: int, max_clip_duration: int, threads: int) -> str:
    ...
    try:
        ...
        raise VideoProcessingError(f"Error combining videos: {str(e)}")
...
def generate_video(combined_video_path: str, tts_path: str, subtitles_path: str, threads: int, subtitles_position: str) -> str:
    ...
    try:
        ...
        raise VideoProcessingError(f"Error generating final video: {str(e)}")
